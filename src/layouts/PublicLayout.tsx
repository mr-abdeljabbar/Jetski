...
import { Menu, X, Phone, MapPin, Instagram, Facebook, Twitter, Mail } from 'lucide-react';
      ...
      <header
        className={`fixed top-0 w-full z-50 transition-all duration-300 ${
          scrolled
            ...
            : 'lg:bg-transparent bg-white/90 backdrop-blur-xl py-4 sm:py-6'
        }`}
      >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-6">
              ...
              <button
                ...
                className="bg-coral text-white text-xs font-bold uppercase tracking-wider px-7 py-3 rounded-full hover:bg-coral/90 transition-all shadow-soft hover:shadow-heavy active:scale-95 cursor-pointer"
              >
                {t('nav_book_now')}
              </button>
            </div>
        ...
        {isMenuOpen && (
          <>
            ...
            >
              ...
              {/* Bottom Actions */}
              <div className="px-8 sm:px-10 py-8 space-y-8 border-t border-ocean/5">
                <div className="space-y-3">
                  <p className="text-[10px] font-black uppercase tracking-[0.25em] text-ocean/40">
                    {t('nav_language')}
                  </p>
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 pb-20 border-b border-white/5">
              ...
              <div className="space-y-8 text-center md:text-left">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-coral">{t('footer_explore')}</h3>
                <ul className="space-y-4">
                  {[
                    { name: t('footer_activities'),    path: '/activities' },
                    { name: t('footer_gallery'),       path: '/gallery'    },
                    { name: t('footer_offers'),        path: '/blog'       },
                    { name: t('footer_our_location'),  path: '/contact'    }
                  ].map((link) => (
                    <li key={link.path}>
              ...
              {/* Support Links */}
              <div className="space-y-8 text-center md:text-left">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-coral">{t('footer_support')}</h3>
                <ul className="space-y-4">
                  {[
                    { name: t('footer_contact'), path: '/contact'        },
                    { name: t('footer_privacy'), path: '/privacy-policy' },
                    { name: t('footer_terms'),   path: '/terms-of-use'   },
                    { name: t('footer_faqs'),    path: '/faqs'           }
                  ].map((link) => (
                    <li key={link.path}>
                      <Link to={link.path} className="text-sky/40 hover:text-white transition-colors flex items-center justify-center md:justify-start group text-sm font-bold uppercase tracking-widest">
                        <span className="w-1.5 h-1.5 rounded-full bg-coral mr-3 opacity-0 group-hover:opacity-100 transition-all hidden md:block"></span>
                  ...
                  <AnimatePresence mode="wait">
                    {!isSubscribed ? (
                      <motion.div
                        ...
                        className="space-y-8"
                      >
                        <h3 className="text-xs font-black uppercase tracking-[0.3em] text-coral">
                        ...
                        >
                          ...
                          >
                            {isNewsletterSubmitting ? t('footer_subscribing') : t('footer_subscribe')}
                          </button>
                        </form>
                      </motion.div>
                    ) : (
                      ...
                      >
                        ...
                        <h3 className="text-2xl font-bold text-white">{t('footer_subscribed_title')}</h3>
                        <p className="text-sky/60 text-sm leading-relaxed max-w-sm">{t('footer_subscribed_body')}</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
            ...
            <div className="pt-12 flex flex-col md:flex-row justify-between items-center gap-10">
              <div className="flex flex-col items-center md:items-start gap-6">
                ...
                <p className="text-sky/20 text-[10px] uppercase tracking-[0.3em] font-bold text-center md:text-left">
                  &copy; {new Date().getFullYear()} Taghazout Jet. {t('footer_copyright')}
                </p>
              </div>
